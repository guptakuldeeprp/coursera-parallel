...
public class ReciprocalArraySum2Test extends TestCase {
    ...
    private double parTestHelper(final int N, final boolean useManyTaskVersion, final int ntasks) {
        ...
        if (useManyTaskVersion) {
            sum = ReciprocalArraySum2.parManyTaskArraySum(input, ntasks);
        } else {
            assert ntasks == 2;
            sum = ReciprocalArraySum2.parArraySum(input);
        }
        ...
        for (int r = 0; r < REPEATS; r++) {
            if (useManyTaskVersion) {
                ReciprocalArraySum2.parManyTaskArraySum(input, ntasks);
            } else {
                assert ntasks == 2;
                ReciprocalArraySum2.parArraySum(input);
            }
        }
